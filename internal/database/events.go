...
func (m *EventModel) Get(id int) (*Event, error) {
	...
	err := m.DB.QueryRowContext(ctx, query, id).Scan(&event.Id, &event.OwnerId, &event.Title, &event.Description, &event.Date, &event.Location)
...
func (m *EventModel) Update(event *Event) error {
	...
	_, err := m.DB.ExecContext(ctx, query, &event.Title, &event.Description, &event.Date, &event.Location, &event.Id)
	if err != nil {
		return err
	}
